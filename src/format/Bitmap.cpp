...
Bitmap::Bitmap(const std::string& filename) {
  ...
  if( info_header->biBitCount == MAX_2_POW_24_COLORS && info_header->biCompression == BI_RGB ) {
    channels_ = 3;
    size_ = channels_ * info_header->biWidth * info_header->biHeight;
    data_ = new unsigned char[size_]; 
    for(unsigned int i=0; i<size_; i++) {
      data_[i] = 0;
    }

    for(unsigned int i=0; i<size_; i+=channels_) {
...
unsigned int Bitmap::get_size() const {
  return size_;
}


unsigned int Bitmap::get_channels() const {
  return channels_;
}
